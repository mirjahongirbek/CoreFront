        ...
        <d-list-group flush class="list-group-small">
          ...
          >
            <router-link :to="'/tables/'+item.projectName">
              <span class="text-semibold text-fiord-blue">{{ item.projectName }}</span>
              <span class="ml-auto text-right text-semibold text-reagent-gray">{{ item.value }}</span>
            </router-link>
            <d-button @click="show('configModal',item.projectName)">SeeConfig</d-button>
          </d-list-group-item>
        </d-list-group>
      </d-card-body>
...
export default {
  ...
  methods: {
    ...
    getAllProjects() {
      this.$api.get("/home/GetallProject").then(
        response => {
